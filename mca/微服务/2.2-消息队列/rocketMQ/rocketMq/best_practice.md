...
这条消息的消费过程中有4次与 DB的 交互，如果按照每次 5ms 计算，那么总共耗时 20ms，假设业务计算耗时 5ms，那么总过耗时 25ms，所以如果能把 4 次 DB 交互优化为 2 次，那么总耗时就可以优化到 15ms，即总体性能提高了 40%。所以应用如果对时延敏感的话，可以把DB部署在SSD硬盘，相比于SCSI磁盘，前者的RT（ Tr : 磁头旋转时间；）会小很多。
...
~~~
1、ASYNC_MASTER（异步主机）
2、SYNC_MASTER（同步主机） --> 适用于对消息可靠性有严格要求
	采用同步刷盘的方式；能够保证消息写入的可靠性；会损失一部分性能
3、SLAVE（从机）
~~~
...
| flushDiskType | ASYNC_FLUSH | 1、SYNC_FLUSH 模式下的 broker 保证在收到确认生产者之前将消息刷盘。<br />2、ASYNC_FLUSH 模式下的 broker 则利用刷盘一组消息的模式，可以取得更好的性能。 |
## 4  NameServer

RocketMQ 中，Name Servers 被设计用来做简单的路由管理。其职责包括：

 ~~~
1、接受 broker 定期的注册请求（注册 + 健康检测）
2、提供最新的路由信息 --》 （生产者，消费者，控制台）
 ~~~
...
```
-  <strong style="color:red">HTTP静态服务器寻址（默认）</strong>
